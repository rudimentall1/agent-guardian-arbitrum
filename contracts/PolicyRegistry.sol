...
contract PolicyRegistry {
    ...
    event PolicyCreated(bytes32 indexed policyId, address indexed owner, bytes32 salt, bytes32 policyHash);
    ...
    /// @notice Create a new, immutable policy owned by the caller. The
    /// actual identifier is derived as `computePolicyId(msg.sender,
    /// salt)` — see contract-level NatSpec, point 5. Reverts if that
    /// identifier has ever been used before, regardless of its prior
    /// active state (same exactly-once semantics as
    ...
    function createPolicy(
        ...
        emit PolicyCreated(policyId, msg.sender, salt, policyHash);
    }
